...
class Config:
    ...
    # 对外推送内容繁体化(香港用户):邮件/企业微信/Telegram/飞书等统一转繁体
    notification_traditional: bool = True
    ...
    # === 投资组合监控（港股/美股）===
    # 依据《AI-Powered Portfolio Monitoring Assistant》提案,对技术/估值/股息三大类
    # 指标按「低/中/高风险规则集」评估并在报告中输出告警。仅对港股/美股生效。
    monitoring_enabled: bool = True            # 是否启用监控评估
    monitoring_risk_tier: str = "medium"       # 风险档位: low(价值/安全) / medium(标准趋势) / high(激进/波动)
    monitoring_market: str = "auto"            # 适用市场: auto(港股+美股) / hk / us
    monitoring_history_days: int = 260         # 拉取的日K历史天数(死叉需≥200根)
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # 如果没有配置，使用默认的示例股票(本系统面向港股/美股)
        if not stock_list:
            stock_list = ['AAPL', 'NVDA', 'HK00700', 'HK09988']
        ...
        return cls(
            ...
            notification_traditional=os.getenv('NOTIFICATION_TRADITIONAL', 'true').lower() == 'true',
            ...
            monitoring_risk_tier=(
                os.getenv('MONITORING_RISK_TIER', 'medium').strip().lower()
                if os.getenv('MONITORING_RISK_TIER', 'medium').strip().lower() in ('low', 'medium', 'high')
                else 'medium'
            ),
            monitoring_market=(
                os.getenv('MONITORING_MARKET', 'auto').strip().lower()
                if os.getenv('MONITORING_MARKET', 'auto').strip().lower() in ('auto', 'hk', 'us')
                else 'auto'
            ),
            monitoring_history_days=int(os.getenv('MONITORING_HISTORY_DAYS', '260')),
        )
