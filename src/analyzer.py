...
class AnalysisResult:
    ...
    # ========== 投资组合监控（港股/美股）==========
    monitoring: Optional[Dict[str, Any]] = None  # 监控规则评估结果(低/中/高风险档位告警)
...
"""
        ...
        # 添加投资组合监控告警（港股/美股:低/中/高风险规则集）
        monitoring = context.get('monitoring')
        if isinstance(monitoring, dict) and monitoring.get('categories'):
            mon_summary = monitoring.get('summary', {})
            alert_lines = [
                f"- 🔴 **{a['display']}**：{a['value_str']} —— {a['rule']}"
                for a in monitoring.get('alerts', [])
            ]
            alert_block = "\n".join(alert_lines) if alert_lines else "- 本档位下暂无触发告警"
            prompt += f"""
### 投资组合监控告警（{monitoring.get('tier_label', '中风险')}）
> 依据《组合监控》提案的风险规则集评估;触发告警 {mon_summary.get('alerts', 0)} 项,数据不可用 {mon_summary.get('unavailable', 0)} 项。
{alert_block}

请在「核心结论」与「风险提示」中结合上述监控告警综合判断(尤其是估值与股息类告警对中长期持有的影响)。
"""
