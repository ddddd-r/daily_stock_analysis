...
class StockAnalysisPipeline:
    ...
    def analyze_stock(self, code: str, report_type: ReportType, query_id: str) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            # Step 3.5: 投资组合监控评估(港股/美股),供两条分析路径共用
            monitoring_block = self._build_monitoring_block(code, stock_name)
            ...
            if use_agent:
                ...
                if agent_result is not None and monitoring_block is not None:
                    agent_result.monitoring = monitoring_block
                return agent_result
            ...
            # Step 7.55: 附加投资组合监控评估块(港股/美股)
            if result and monitoring_block is not None:
                result.monitoring = monitoring_block
    ...
    def _build_monitoring_block(self, code: str, stock_name: str = "") -> Optional[Dict[str, Any]]:
        """
        构建「投资组合监控」评估块(港股/美股)。

        依据提案的低/中/高风险规则集,对技术 / 基本面 / 股息三大类指标逐项评估,
        输出告警结构供报告渲染与 LLM 参考。仅对港股 / 美股运行;fail-open。
        """
        cfg = self.config
        if not getattr(cfg, "monitoring_enabled", True):
            return None

        from data_provider.base import _market_tag
        market = _market_tag(code)
        pref = str(getattr(cfg, "monitoring_market", "auto") or "auto").lower()
        if pref in ("hk", "us") and market != pref:
            return None
        if market not in ("hk", "us"):
            return None  # 监控体系面向港股 / 美股

        try:
            from src.monitoring import evaluate_monitoring
            days = int(getattr(cfg, "monitoring_history_days", 260))
            df, _src = self.fetcher_manager.get_daily_data(code, days=days)
            fund = self.fetcher_manager.get_overseas_fundamental_metrics(code)
            tier = str(getattr(cfg, "monitoring_risk_tier", "medium"))
            block = evaluate_monitoring(df, fund, tier=tier)
            block["code"] = code
            block["name"] = stock_name or (fund or {}).get("name") or code
            block["market"] = market
            block["fundamentals"] = fund
            logger.info(
                f"{stock_name}({code}) 监控评估: 档位={block['tier_label']}, "
                f"告警={block['summary']['alerts']}, 数据不可用={block['summary']['unavailable']}"
            )
            return block
        except Exception as e:
            logger.warning(f"{stock_name}({code}) 监控评估失败: {e}")
            return None
    ...
    def _enhance_context(
        ...
        monitoring_block: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """
        增强分析上下文
        ...
        # 投资组合监控评估块(港股/美股):供 analyzer 提示词参考触发的告警
        if isinstance(monitoring_block, dict):
            enhanced["monitoring"] = monitoring_block
