...
class NotificationService(
    ...
    def generate_dashboard_report(
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                # ========== 投资组合监控仪表盘（港股/美股）==========
                self._append_monitoring_section(report_lines, result)
    ...
    # 监控指标状态 -> 显示标记
    _MONITORING_STATUS_EMOJI = {
        "alert": "🔴",
        "ok": "🟢",
        "unavailable": "⚪",
        "ignored": "⚫",
    }
    _MONITORING_STATUS_LABEL = {
        "alert": "告警",
        "ok": "正常",
        "unavailable": "数据不可用",
        "ignored": "本档忽略",
    ...
    def _append_monitoring_section(self, lines: List[str], result: AnalysisResult) -> None:
        """渲染「投资组合监控仪表盘」(港股/美股):三大类指标 + 低/中/高风险规则告警。"""
        monitoring = getattr(result, "monitoring", None)
        if not isinstance(monitoring, dict) or not monitoring.get("categories"):
            return

        summary = monitoring.get("summary", {})
        n_bars = monitoring.get("n_bars", 0)
        lines.extend([
            "### 📐 投资组合监控仪表盘",
            "",
            f"> 风险档位: **{monitoring.get('tier_label', '中风险')}** | "
            f"🔴 告警 {summary.get('alerts', 0)} | "
            f"⚪ 数据不可用 {summary.get('unavailable', 0)} | "
            f"基于 {n_bars} 根日K线",
            "",
        ])
        ...
        if alerts:
            ...
            lines.append("")
            for a in alerts:
                lines.append(f"- **{a['display']}**: {a['value_str']} —— {a['rule']}")
            lines.append("")

        # 各类别明细表
        for cat in monitoring.get("categories", []):
            ...
            for i in indicators:
                emoji = self._MONITORING_STATUS_EMOJI.get(i["status"], "⚪")
                label = self._MONITORING_STATUS_LABEL.get(i["status"], i["status"])
                note = f" *（{i['note']}）*" if i.get("note") else ""
                lines.append(
                    f"| {emoji}{label} | {self._escape_md(i['display'])} | "
                    f"{self._escape_md(str(i['value_str']))} | {self._escape_md(i['rule'])}{note} |"
                )
            lines.append("")

        # 暂未实现的类别(衍生品与流动性)
        for d in monitoring.get("deferred", []):
            lines.append(f"> ⏸️ **{d.get('label', '')}**: {d.get('reason', '')}")
        lines.append("")
    ...
    def send(
        ...
        # 对外推送统一追加免责声明(非投资建议)
        try:
            from src.disclaimer import REPORT_DISCLAIMER
            if REPORT_DISCLAIMER not in content:
                content = f"{content}\n\n---\n> {REPORT_DISCLAIMER}"
        except Exception as exc:  # noqa: BLE001 - fail-open
            logger.debug("追加免责声明跳过: %s", exc)

        # 对外推送内容繁体化(香港用户):在统一发送入口转换,覆盖全部渠道与图片渲染
        try:
            from src.config import get_config
            if getattr(get_config(), "notification_traditional", True):
                from src.utils.zh_convert import to_traditional
                content = to_traditional(content)
        except Exception as exc:  # noqa: BLE001 - fail-open
            logger.debug("繁体转换跳过: %s", exc)
