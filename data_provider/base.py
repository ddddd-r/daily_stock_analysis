...
class DataFetcherManager:
    ...
    def __init__(self, fetchers: Optional[List[BaseFetcher]] = None):
        ...
        self._fundamental_yf_adapter = YfinanceFundamentalAdapter()
    ...
    def get_overseas_fundamental_metrics(self, stock_code: str, include_capex: bool = True) -> Dict[str, Any]:
        """
        返回港股 / 美股的扁平基本面指标(来自 yfinance adapter)。

        供「投资组合监控」框架直接取用估值 / 增长 / 股息原始数值。
        """
        return self._fundamental_yf_adapter.get_metrics(stock_code, include_capex=include_capex)

    def _build_overseas_fundamental_context(self, stock_code: str, market: str) -> Dict[str, Any]:
        """
        将 yfinance 扁平指标映射为标准 fundamental_context 结构(港股 / 美股)。

        valuation / growth 块填入可得数据;海外不支持的块(institution /
        capital_flow / dragon_tiger / boards)标记 not_supported。
        """
        start_ts = time.time()
        metrics = self._fundamental_yf_adapter.get_metrics(stock_code)
        elapsed_ms = int((time.time() - start_ts) * 1000)
        status = str(metrics.get("status", "failed"))
        errors = list(metrics.get("errors", []))
        chain = [{"provider": "yfinance_fundamental", "result": status, "duration_ms": elapsed_ms}]

        valuation_payload = {
            "pe_ratio": metrics.get("trailing_pe"),
            "pb_ratio": metrics.get("pb"),
            "forward_pe": metrics.get("forward_pe"),
            "peg": metrics.get("peg"),
            "dividend_yield_pct": metrics.get("dividend_yield_pct"),
            "payout_ratio_pct": metrics.get("payout_ratio_pct"),
        ...
        growth_payload = {
            "forecast_earnings_growth_pct": metrics.get("earnings_growth_pct"),
            "revenue_growth_pct": metrics.get("revenue_growth_pct"),
        }

        valuation_status = self._infer_block_status(valuation_payload, status)
        growth_status = self._infer_block_status(growth_payload, status)

        not_supported_block = lambda: self._build_fundamental_block(  # noqa: E731
            "not_supported",
            {},
            [{"provider": "yfinance_fundamental", "result": "not_supported", "duration_ms": 0}],
            ["overseas market: block not supported"],
        )

        result_ctx: Dict[str, Any] = {
            "market": market,
            "valuation": self._build_fundamental_block(valuation_status, valuation_payload, chain, errors),
            "growth": self._build_fundamental_block(growth_status, growth_payload, chain, errors),
            "earnings": not_supported_block(),
            "institution": not_supported_block(),
            "capital_flow": not_supported_block(),
            "dragon_tiger": not_supported_block(),
            "boards": not_supported_block(),
        }
        block_statuses = {block: result_ctx[block]["status"] for block in (
            "valuation", "growth", "earnings", "institution",
            "capital_flow", "dragon_tiger", "boards",
        )}
        result_ctx["coverage"] = block_statuses
        result_ctx["source_chain"] = chain
        result_ctx["errors"] = errors
        if all(v == "not_supported" for v in block_statuses.values()):
            result_ctx["status"] = "not_supported"
        elif valuation_status == "ok" or growth_status == "ok":
            result_ctx["status"] = "partial"
        else:
            result_ctx["status"] = "partial" if status != "failed" else "failed"
        result_ctx["elapsed_ms"] = elapsed_ms
        return result_ctx
    ...
    def get_fundamental_context(
        ...
        if market in {"us", "hk"}:
            # 港股 / 美股:走 yfinance 基本面 adapter(估值 / 增长 / 股息)
            return self._build_overseas_fundamental_context(stock_code, market)
