...
/** 监控指标状态 */
export type MonitoringStatus = 'alert' | 'ok' | 'unavailable' | 'ignored';
...
/** 暂未实现的监控类别（衍生品与流动性） */
export interface MonitoringDeferred {
  key: string;
  label: string;
  reason: string;
  indicators?: string;
...
export interface ReportMonitoring {
  ...
  alerts: Array<MonitoringIndicator & { category: string }>;
  deferred: MonitoringDeferred[];
  summary: { evaluated: number; alerts: number; unavailable: number };
}
