...
class TestFundamentalContext(unittest.TestCase):
    def test_overseas_market_uses_yfinance_fundamentals(self) -> None:
        # 港股/美股现走 yfinance 基本面 adapter:估值/增长有数据,
        # 其余 A 股专属块(earnings/institution/capital_flow/dragon_tiger/boards)标记 not_supported。
        ...
        fake_metrics = {
            "status": "ok", "name": "Apple Inc.", "sector": "Technology", "currency": "USD",
            "forward_pe": 30.0, "trailing_pe": 35.0, "peg": 2.3, "pb": 40.0,
            "earnings_growth_pct": 21.0, "revenue_growth_pct": 16.0,
            "dividend_yield_pct": 0.4, "payout_ratio_pct": 12.0, "capex_pct_revenue": 3.0,
            "errors": [],
        }
        with patch("src.config.get_config", return_value=cfg), \
                patch.object(manager._fundamental_yf_adapter, "get_metrics", return_value=fake_metrics):
        ...
        self.assertEqual(ctx["valuation"]["data"]["pb_ratio"], 40.0)
        self.assertEqual(ctx["valuation"]["data"]["dividend_yield_pct"], 0.4)
        self.assertEqual(ctx["growth"]["data"]["forecast_earnings_growth_pct"], 21.0)
        # 海外不支持的块
